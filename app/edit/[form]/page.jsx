...
function Page({params}) {
    let {setFormData,setImage,loading,getRow,row,editForm} = useContext(Appcontext);
    useEffect(()=>{
        getRow(params?.form)
    },[])
    // console.log(row)
    ...
    return (
        <div className='w-full flex flex-col gap-5 justify-center items-center my-10'>
            ...
            <div className='w-1/2 flex flex-row justify-between'>
                ...
                {step !== 2 ? <button type="button" onClick={handleNext} class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Next</button> : <button type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" onClick={()=>editForm(row[0]?.id)} style={loading?{backgroundColor:'gray',cursor:'not-allowed'}:{}} disabled={loading}>Finish & Edit</button>}
            </div>
        </div>
